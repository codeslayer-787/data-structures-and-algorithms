...
const containsWorld = (input) => {
  // Solution code here...
  const lowerCaps = /world/gm.test(input);
  if(lowerCaps){
    return true;
  }else
  return false;
...
const isCapitalized = (str) => {
  // Solution code here...
  const isIt = str.match(/[A-Z][a-z]+/gm);
  if(isIt){
    return isIt;
  }else
  return [];
...
const citiesAtoJ = (arr) => {
  ...
  arr.forEach(value => {
    ...
    const aThroughJ = value.match(/^[A-J]\w+/gm);
    if(aThroughJ) {
      newArr.push(aThroughJ[0]);
    }
  });
  return newArr;
};
