...
// Solution code here...
let arrOfStrings = arr.map(value => value.length);
return arrOfStrings.indexOf(Math.max(...arrOfStrings));
};
...
const findHappiness = (arr) => {
  ...
  return arr.filter(value => {
    let happyPhrases = /(:)\)/gm
    return happyPhrases.test(value);
  })
};
...
const onlyOddChars = (str) => {
  // Solution code here...
  return str.split('').filter((val, idx)=> idx % 2 !== 0).join('');
};
...
const allHappy = (arr) => {
  // Solution code here...
  return !arr.map(val => /(\:\))/gm.test(val)).includes(false);
};
